...
describe("category, tag, and merchant management", () => {
  ...
  beforeAll(async () => {
    ...
    insertTestUser(sql, { id: "owner", displayName: "Owner" });
    insertTestUser(sql, { id: "other", displayName: "Other" });
    sql.prepare(
      `INSERT INTO accounts (id, workspace_id, name, type, opening_balance_minor, opening_balance_date)
       VALUES ('account', 'owner', 'Current', 'current', 100000, '2026-07-01')`,
    ).run();
    sql.prepare(
      `INSERT INTO categories (id, workspace_id, parent_id, name, kind, spending_nature, spending_priority)
       VALUES
    ...
    sql.prepare(
      `INSERT INTO merchants (id, workspace_id, name, normalized_name)
       VALUES ('merchant', 'owner', 'Old Shop', 'old shop'), ('other-merchant', 'other', 'Private Shop', 'private shop')`,
    ).run();
    sql.prepare(
      `INSERT INTO transactions
         (id, workspace_id, account_id, merchant_id, kind, status, amount_minor, occurred_at)
